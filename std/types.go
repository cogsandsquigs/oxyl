...
func (t *Type) CmpOpType(op Operator, typ string) bool {
...
func (t *Type) ExecuteOp(op Operator, l, r *Value) (*Value, error) {
	if _, ok := t.cmpoptypes[op]; !ok {
		return NoneValue(), fmt.Errorf("execution error: operator " + string(op) + " does not exist")
	} else if l.typ != t.name {
		return NoneValue(), fmt.Errorf("execution error: left operand of type " + l.typ + " is not equal to expected type " + t.name)
	} else if !t.CmpOpType(op, r.typ) {
		return NoneValue(), fmt.Errorf("execution error: the operator " + string(op) + " is not defined for types " + t.name + " and " + r.typ)
	}
	i, s := t.execopfuncs[op](l, r)
	return NewValue(s, i), nil
}
...
var Int = &Type{
	...
	execopfuncs: map[Operator]func(l, r *Value) (interface{}, string){
		ADD: func(l, r *Value) (interface{}, string) {
			...
			case "Float":
				return float64(l.val.(int64)) + r.val.(float64), "Float"
			default:
				return nil, "None"
			}
		},
...
var Float = &Type{
	...
	execopfuncs: map[Operator]func(l, r *Value) (interface{}, string){
		ADD: func(l, r *Value) (interface{}, string) {
			...
			case "Float":
				return l.val.(float64) + r.val.(float64), "Float"
			default:
				return nil, "None"
			}
		},
